...
public class CDKTest
{
	...
	private static String getInchiForSmiles(String smiles) throws InvalidSmilesException, CDKException
	{
		return InChIGeneratorFactory.getInstance()
				.getInChIGenerator(new SmilesParser(SilentChemObjectBuilder.getInstance()).parseSmiles(smiles))
				.getInchi();
	}
	...
	{
		{
			...
			Assert.assertEquals(getInchiForSmiles("O=S(=O)(c1ccc(cc1)n2ccnc2(cc))C"), getInchiForSmiles(data
					.getClusters().get(0).getSubstructureSmarts(SubstructureSmartsType.MCS)));
	...
	@Test
	public void testSmilesAndCDKProps() throws Exception
	{
		String data[] = { "data/caco2_20.csv", "data/cox2_13_mcs.sdf", "data/mcs.smi" };
		boolean smilesProp[] = { true, false, true };
		boolean cdkTitle[] = { false, true, false };
		boolean cdkRemark[] = { false, false, false };
		for (int i = 0; i < smilesProp.length; i++)
		{
			...
			for (IntegratedPropertySet p : d.getIntegratedProperties())
			{
				hasTitle |= p.toString().equals("cdk:Title");
				hasRemark |= p.toString().equals("cdk:Remark");
				hasSmiles |= p.isSmiles();
			}
			Assert.assertEquals(smilesProp[i], hasSmiles);
			Assert.assertEquals(cdkTitle[i], hasTitle);
			Assert.assertEquals(cdkRemark[i], hasRemark);
		}
	}
